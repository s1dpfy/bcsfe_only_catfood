...
class BCSFEService:
    ...
    def set_catfood(self, value: int): self._require_save().catfood = min(value, core.core_data.max_value_manager.catfood)
    ...
    def set_normal_tickets(self, value: int): self._require_save().normal_tickets = min(value, core.core_data.max_value_manager.normal_tickets)
    ...
    def set_platinum_tickets(self, value: int): self._require_save().platinum_tickets = min(value, core.core_data.max_value_manager.platinum_tickets)

    def get_legend_tickets(self): return self._require_save().legend_tickets
    def set_legend_tickets(self, value: int): self._require_save().legend_tickets = min(value, core.core_data.max_value_manager.legend_tickets)
    ...
    def set_catfruit(self, values: list[int]): 
        self._require_save().catfruit = [min(v, 998) for v in values] # 최대 998 고정 제한
    ...
    # 🔥 핵심 요약 로직 (프론트에 현재값과 MAX값을 함께 보내줍니다)
    def summary(self):
    ...
    def search_cats_by_name(self, query: str) -> list[dict]:
        """이름으로 고양이 목록을 검색하여 반환합니다."""
        save_file = self._require_save()
        # BCSFE 내부의 get_cats_name 함수를 활용
    ...
    # (위쪽 코드는 그대로 두시고, 맨 아래 upload 함수 밑의 두 함수만 교체하세요)

    def upload(self) -> tuple[str, str]:
        return self._require_handler().get_codes()

    # ▼ 추가/수정된 파일 직접 불러오기 함수 ▼
    def load_file(self, file_path: str) -> bool:
        try:
            # 1. 파일 경로를 bcsfe 전용 Path 객체로 변환
            path = core.Path(file_path)
            
            # 2. 파일에서 실제 데이터를 읽어옴 (Data 객체 반환)
            data = path.read()
            
            # 3. 읽어온 데이터를 SaveFile에 집어넣음
    ...
    def save_to_file(self, path: str):
        # 여기도 마찬가지로 core.Path 객체로 감싸서 넘겨줍니다.
