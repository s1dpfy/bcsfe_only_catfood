...
def main():
    ...
    # 중복된 입력 프롬프트를 제거하고 레전드 티켓 입력을 포함했습니다.
    ...
    new_rare_tickets = get_int_input("레어 티켓", service.get_rare_tickets())
    new_legend_tickets = get_int_input("레전드 티켓", service.get_legend_tickets())
    new_leadership = get_int_input("리더십", service.get_leadership())

    # 입력받은 값을 실제 객체에 반영합니다.
    service.set_catfood(new_catfood)
    service.set_xp(new_xp)
    service.set_np(new_np)
    service.set_rare_tickets(new_rare_tickets)
    service.set_legend_tickets(new_legend_tickets)
